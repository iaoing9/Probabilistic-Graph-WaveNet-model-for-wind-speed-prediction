...
class Config:
    ...
    output_dir = os.path.join(root_dir, "GWN_Dataset1_MSE_results")
    ...
    # This ablation replaces GNLL training with MSE training and does not output post-hoc variance.
    eps_var = 1e-6
    interval_alpha = 0.05
...
class DeterministicGWN(nn.Module):
    def __init__(
        self,
        num_nodes,
        input_features,
...
def compute_all_metrics(y_true, mu):
    metrics = {
        "RMSE": rmse(y_true, mu),
        "MAE": mae(y_true, mu),
...
def inverse_y_and_prediction(y_norm, mu_norm, scaler_y):
...
def train_one_horizon(cfg, transformed_data, scaler_y, horizon_id):
    ...
    best_model_path = os.path.join(horizon_dir, f"best_mse_gwn_horizon_{horizon_id:02d}.pt")
    ...
    for split_name in ["Train", "Validation", "Test"]:
        y_norm, mu_norm = predict(model, eval_loaders[split_name], cfg.device)

        y, mu = inverse_y_and_prediction(
            y_norm=y_norm,
            mu_norm=mu_norm,
            scaler_y=scaler_y
        )
        ...
        metrics["Best_validation_MSE"] = best_val_loss
