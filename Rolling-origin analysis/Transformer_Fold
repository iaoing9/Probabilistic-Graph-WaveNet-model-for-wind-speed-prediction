...
class Config:
    ...
    output_dir = os.path.join(root_dir, "Transformer_Dataset1_Fold1_results")
...
def train_one_horizon(cfg, transformed_data, scaler_y, horizon_id):
    ...
    best_model_path = os.path.join(horizon_dir, f"best_probabilistic_transformer_horizon_{horizon_id:02d}.pt")

    print("\n" + "=" * 80)
    print(f"Training Transformer Horizon {horizon_id:02d}: {lead_hours} h ahead, lead_steps = {lead_steps} samples")
