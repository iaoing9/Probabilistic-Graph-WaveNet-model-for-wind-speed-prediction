...
class Config:
    ...
    output_dir = os.path.join(root_dir, "Persistence_Dataset1_Fold1_results")

    seed = 2026

    # GPU setting is retained only for interface consistency.
    # The persistence baseline itself is non-parametric and runs on CPU.
    ...
    # Input window length. The default 48 h window corresponds to 144 rows.
    ...
    #   Persistence mean is the last observed bridge-site wind speed in the input window.
    # "same_target_time": all horizons are evaluated on exactly the same target timestamps.
    #   Persistence mean is still the last observed bridge-site wind speed in the input window.
    window_alignment = "same_input_window"

    # Grid and feature settings. These are retained for data-interface consistency.
    num_nodes = 9
    num_grid_features = 3
    ...
    # The prediction mean is the last historical observed bridge-site wind speed
    # in the input window. The predictive variance is estimated from training
    # residuals separately for each prediction horizon.
    persistence_reference = "last_observed_bridge_site_wind_speed_in_input_window"

    # Training-related settings are retained only for interface consistency.
    batch_size = 16
    epochs = 1
    learning_rate = 1e-3
    weight_decay = 1e-5
    ...
    # Persistence has no stochastic training. Keep False to preserve deterministic order.
    shuffle_train = False
...
# =========================================================
# These definitions are retained from the GWN code for file-output consistency.
# The persistence baseline does not use spatial graph information.
...
# =========================================================
# 5. Probabilistic persistence baseline
# =========================================================

class ProbabilisticPersistence:
    ...
    @staticmethod
    def _persistence_mean_from_batch(x_global):
        """
        x_global: [B, T, 1]
        return:   [B, 1]

        Persistence prediction uses the last observed bridge-site wind speed
        in the input window as the forecast mean for the selected future horizon.
        """
        return x_global[:, -1, :]
    ...
    def fit(self, loader):
        ...
        for _, x_global, y in loader:
            mu = self._persistence_mean_from_batch(x_global)
        ...
        if residual.shape[0] > 1:
            error_var = np.var(residual, axis=0, ddof=1, keepdims=True)
        else:
            error_var = np.var(residual, axis=0, ddof=0, keepdims=True)

        error_var = np.maximum(error_var, self.eps_var).astype(np.float32)
        self.error_var = error_var
        self.error_std = np.sqrt(error_var).astype(np.float32)

        return self

    def predict(self, loader):
        if self.error_var is None:
            raise RuntimeError("The probabilistic persistence baseline must be fitted before prediction.")
        ...
        for _, x_global, y in loader:
            ...
            mu = self._persistence_mean_from_batch(x_global).numpy()
            var = np.repeat(self.error_var, batch_size, axis=0)
    ...
    def evaluate_gnll(self, loader):
        y, mu, var = self.predict(loader)
        var = np.maximum(var, self.eps_var)
        loss = 0.5 * (np.log(var) + (y - mu) ** 2 / var)
...
def train_one_horizon(cfg, transformed_data, scaler_y, horizon_id):
    ...
    model = ProbabilisticPersistence(eps_var=cfg.eps_var)
    model.fit(loaders["Train"])

    train_gnll = model.evaluate_gnll(loaders["Train"])
    val_gnll = model.evaluate_gnll(loaders["Validation"])
    ...
    history_df = pd.DataFrame([{
        ...
        "Epoch": 0,
        "Train_GNLL": train_gnll,
        "Validation_GNLL": val_gnll,
        "Note": "Probabilistic persistence baseline: prediction mean is the last observed bridge-site wind speed in the input window; variance is estimated from training residuals for this horizon."
    }])
    history_df.to_excel(
    ...
    y_std = float(np.squeeze(scaler_y.std))
    error_var_original_scale = model.error_var.reshape(-1) * (y_std ** 2)

    variance_df = pd.DataFrame({
        "Horizon_step": [horizon_id],
        "Lead_hours": [lead_hours],
        "Lead_samples": [lead_steps],
        "Persistence_error_variance_normalized": model.error_var.reshape(-1),
        "Persistence_error_std_normalized": model.error_std.reshape(-1),
        "Persistence_error_variance_original_scale": error_var_original_scale,
        "Persistence_error_std_original_scale": np.sqrt(error_var_original_scale),
        "Persistence_reference": cfg.persistence_reference,
    })
    variance_df.to_excel(
        os.path.join(horizon_dir, f"persistence_error_variance_horizon_{horizon_id:02d}.xlsx"),
        index=False
    )

    print("\n" + "=" * 80)
    print(f"Persistence Horizon {horizon_id:02d}: {lead_hours} h ahead, lead_steps = {lead_steps} samples")
    ...
    print(f"Persistence reference = {cfg.persistence_reference}")
    for split_name in ["Train", "Validation", "Test"]:
        print(f"{split_name} samples: {len(datasets[split_name])}")
    print(f"Train GNLL = {train_gnll:.6f} | Validation GNLL = {val_gnll:.6f}")
    print("=" * 80)
    ...
    for split_name in ["Train", "Validation", "Test"]:
        y_norm, mu_norm, var_norm = model.predict(eval_loaders[split_name])
        ...
        metrics["Best_validation_GNLL"] = val_gnll
        metrics["Persistence_reference"] = cfg.persistence_reference
        ...
        for k, v in metrics.items():
            if isinstance(v, float):
                print(f"{k}: {v:.6f}")
            elif k in ["Split", "Horizon_step", "Lead_hours", "Effective_sample_size", "Best_epoch", "Persistence_reference"]:
                print(f"{k}: {v}")
...
def save_static_information(cfg, raw_data, scaler_grid, scaler_global, scaler_y):
    ...
    feature_info = pd.DataFrame({
        ...
        "Excel_columns": [
            ...
            "Last AC value within each input window",
        ],
        "Used_as": [
            "grid input retained for interface consistency",
            "grid input retained for interface consistency",
            "grid input retained for interface consistency",
            "global historical input",
            "prediction target",
            "probabilistic persistence mean",
        ],
    })
...
def train_all_horizons(cfg):
    ...
    print("Persistence baseline does not require CUDA. Device setting is retained for output consistency.")
