...
class Config:
    ...
    output_dir = os.path.join(root_dir, "GWN_Dataset1_ANE_only_results")
    ...
    # AC-only input setting:
    # Only the historical bridge-site wind speed in column AC is used as model input.
    # ECMWF columns B:AB are not used. Therefore, the spatial graph degenerates to
    # one virtual station node and no grid feature is provided.
    num_nodes = 1
    num_grid_features = 0
...
# In the AC-only setting, spatial dependence among ECMWF grid points is not used.
# A single virtual station node is retained only to keep the original GWN code structure.
grid_coords = [
    station_coord,
]
...
def build_distance_adjacency(coords, sigma_scale=1.0):
    ...
    # In the AC-only setting, there is only one virtual station node.
    # The spatial adjacency therefore degenerates to a 1 x 1 identity matrix.
...
def read_dataset1_excel(file_path):
    ...
    Inputs in this AC-only version:
    x_grid   = empty grid feature array with one virtual station node, [T, 1, 0]
    x_global = AC, historical observed bridge-site wind speed, [T, 1]
    y        = AC, target observed bridge-site wind speed, [T, 1]

    ECMWF columns B:AB are intentionally not used as model input.
    """
    ...
    # No ECMWF grid feature is used in this AC-only input setting.
    # A one-node empty grid tensor is retained only for interface consistency.
    x_grid = np.zeros((len(ane_raw), 1, 0), dtype=np.float32)
...
class StandardScaler:
    ...
    def fit(self, data, axis=None):
        ...
        # Empty arrays occur in the AC-only setting because no ECMWF grid feature
        # is used. Keep a broadcastable zero-mean/unit-std placeholder.
        if data.size == 0:
            if axis is None:
                shape = tuple(1 for _ in range(data.ndim))
            else:
                if isinstance(axis, int):
                    axis_tuple = (axis,)
                else:
                    axis_tuple = tuple(axis)
                axis_tuple = tuple(a if a >= 0 else data.ndim + a for a in axis_tuple)
                shape = tuple(1 if i in axis_tuple else data.shape[i] for i in range(data.ndim))
...
def train_one_horizon(cfg, transformed_data, scaler_y, horizon_id):
    ...
    best_model_path = os.path.join(horizon_dir, f"best_probabilistic_gwn_horizon_{horizon_id:02d}.pt")
