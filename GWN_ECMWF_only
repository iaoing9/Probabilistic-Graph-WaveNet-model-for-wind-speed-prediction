...
class Config:
    ...
    output_dir = os.path.join(root_dir, "GWN_Dataset1_ECMWF_only_results")
    ...
    # ECMWF-only input setting:
    # only columns B:AB are used as model input.
    # Column AC is used only as the prediction target and is no longer used
    # as a historical global input feature.
    num_nodes = 9
    num_grid_features = 3
...
def read_dataset1_excel(file_path):
    ...
    x_global = empty array with shape [T, 0]; no historical AC is used as input
    ...
    # ECMWF-only setting: AC is used only as the prediction target.
    # It is not used as a historical model input.
    x_global = np.zeros((len(ane_raw), 0), dtype=np.float32)
...
class StandardScaler:
    ...
    def fit(self, data, axis=None):
        # Support empty global-feature arrays with shape [T, 0].
        # This is used in the ECMWF-only input setting.
        if data.shape[-1] == 0:
            if axis is None:
                ...
                for ax in axes:
                    out_shape[ax] = 1
                self.mean = np.zeros(out_shape, dtype=np.float32)
                self.std = np.ones(out_shape, dtype=np.float32)
            return
...
class ProbabilisticGWN(nn.Module):
    ...
    def forward(self, x_grid, x_global):
        """
        x_grid:   [B, T, N, F_grid]
        x_global: [B, T, F_global]; F_global = 0 in the ECMWF-only setting
        ...
        if x_global is not None and x_global.size(-1) > 0:
...
def save_static_information(cfg, raw_data, scaler_grid, scaler_global, scaler_y):
    ...
    for name, scaler in [
        ...
        ("global_feature_not_used", scaler_global),
    ...
    feature_info = pd.DataFrame({
        ...
        "Used_as": [
            ...
            "grid input",
            "grid input",
            "not used as model input in ECMWF-only setting",
            "prediction target",
        ],
    })
...
def train_all_horizons(cfg):
    ...
    print("Input setting: ECMWF-only. Columns B:AB are used as input; column AC is used only as target.")
