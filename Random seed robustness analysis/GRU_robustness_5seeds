...
class Config:
    ...
    output_dir = os.path.join(root_dir, "GRU_Dataset1_robustness_5seeds_results")
...
def train_one_horizon(cfg, transformed_data, scaler_y, horizon_id, run_seed, run_output_dir):
    ...
    best_model_path = os.path.join(
        horizon_dir,
        f"best_probabilistic_gru_seed_{run_seed}_horizon_{horizon_id:02d}.pt"
    )

    print("\n" + "=" * 80)
    print(f"Seed {run_seed} | Training GRU Horizon {horizon_id:02d}: {lead_hours} h ahead, lead_steps = {lead_steps} samples")
...
def train_all_horizons(cfg):
    ...
    for run_seed in cfg.random_seeds:
        print("\n" + "=" * 80)
        print(f"Running GRU robustness experiment with random seed = {run_seed}")
        print("=" * 80)
