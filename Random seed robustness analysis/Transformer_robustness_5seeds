...
class Config:
    ...
    output_dir = os.path.join(root_dir, "Transformer_Dataset1_robustness_5seeds_results")
    ...
    # This probabilistic Transformer baseline keeps the same input data and
    # probabilistic output formulation as the GWN model, but removes graph
    # convolution and spatial adjacency. The 9 ECMWF grid-node features are
    # flattened as ordinary sequence features and concatenated with the
    # historical bridge-site wind-speed feature.
    d_model = 64
    nhead = 4
...
class ProbabilisticTransformer(nn.Module):
    def __init__(
        ...
        # Transformer does not use graph convolution or spatial adjacency.
        # The ECMWF grid-node features are flattened as ordinary sequence features.
        self.input_dim = num_nodes * num_grid_features + num_global_features

        self.input_projection = nn.Linear(self.input_dim, d_model)
        self.positional_encoding = PositionalEncoding(
            d_model=d_model,
            max_len=max(input_len, 5000),
            dropout=dropout
        )
        ...
        self.raw_var_head = nn.Linear(d_model, 1)
    ...
    def forward(self, x_grid, x_global):
        ...
        # Flatten the 9 ECMWF grid nodes and 3 variables into ordinary features.
        # This keeps the Transformer baseline free from explicit spatial graph modeling.
...
def train_one_horizon(cfg, transformed_data, scaler_y, horizon_id, run_seed, run_output_dir):
    ...
    best_model_path = os.path.join(horizon_dir, f"best_probabilistic_transformer_seed_{run_seed}_horizon_{horizon_id:02d}.pt")

    print("\n" + "=" * 80)
    print(f"Seed {run_seed} | Training Transformer Horizon {horizon_id:02d}: {lead_hours} h ahead, lead_steps = {lead_steps} samples")
...
def save_static_information(cfg, raw_data, scaler_grid, scaler_global, scaler_y):
    ...
    feature_info = pd.DataFrame({
        ...
        "Used_as": [
            "grid input flattened for Transformer",
            "grid input flattened for Transformer",
            "grid input flattened for Transformer",
    ...
    model_info_df = pd.DataFrame({
        ...
        "Value": [
            ...
            "Flattened grid-node features plus global historical observation; no graph convolution or adaptive adjacency",
            cfg.num_nodes,
...
def train_all_horizons(cfg):
    ...
    for run_seed in cfg.random_seeds:
        print("\n" + "=" * 80)
        print(f"Running Transformer robustness experiment with random seed = {run_seed}")
        print("=" * 80)
