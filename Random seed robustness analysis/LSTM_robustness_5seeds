...
class Config:
    ...
    output_dir = os.path.join(root_dir, "LSTM_Dataset1_robustness_5seeds_results")
    ...
    # LSTM architecture
    # This probabilistic LSTM baseline keeps the same input data and probabilistic
    # output formulation as the GWN model, but removes graph convolution and
...
def set_seed(seed: int):
    ...
    try:
        # Robustness analysis uses different random seeds for repeated runs.
        # The following setting keeps the training speed consistent with the original LSTM script.
...
class ProbabilisticLSTM(nn.Module):
    def __init__(
        ...
        # LSTM does not use graph convolution or spatial adjacency.
        # The ECMWF grid-node features and the broadcast ANE historical feature
        # are flattened as ordinary sequential features.
        self.lstm_input_dim = num_nodes * input_features
    ...
    def forward(self, x_grid, x_global):
        ...
        # Broadcast the historical bridge-site ANE feature to all ECMWF grid nodes.
        # This keeps the same input representation as the probabilistic GWN model,
        # except that the spatial node-feature dimensions are flattened for LSTM.
        x_global_expand = x_global.unsqueeze(2).repeat(1, 1, n, 1)
        x = torch.cat([x_grid, x_global_expand], dim=-1)

        # [B, T, N, F] to [B, T, N * F]
        x = x.reshape(b, t, n * x.shape[-1]).contiguous()
...
def train_one_horizon(cfg, transformed_data, scaler_y, horizon_id, run_seed, run_output_dir):
    ...
    best_model_path = os.path.join(horizon_dir, f"best_probabilistic_lstm_seed_{run_seed}_horizon_{horizon_id:02d}.pt")
...
def save_static_information(cfg, raw_data, scaler_grid, scaler_global, scaler_y):
    ...
    model_info_df = pd.DataFrame({
        ...
        "Value": [
            "Probabilistic LSTM",
            "Flattened node-feature sequence without graph convolution",
            cfg.num_nodes,
            cfg.num_grid_features,
            cfg.num_global_features,
            "Gaussian mean and variance",
            "No explicit graph convolution or adaptive adjacency",
            f"Repeated over {len(cfg.random_seeds)} random seeds: {cfg.random_seeds}",
        ],
    })
...
def train_all_horizons(cfg):
    ...
    for run_seed in cfg.random_seeds:
        print("\n" + "=" * 80)
        print(f"Running LSTM robustness experiment with random seed = {run_seed}")
        print("=" * 80)
