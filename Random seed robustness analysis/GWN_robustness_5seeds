...
class Config:
    ...
    output_dir = os.path.join(root_dir, "GWN_Dataset1_robustness_5seeds_results")
...
def set_seed(seed: int):
    ...
    try:
        # Robustness analysis requires reproducible runs for each random seed.
...
class GraphConv(nn.Module):
    def __init__(self, c_in, c_out, dropout, support_len, order):
        ...
        c_in_total = c_in * (order * support_len + 1)
        self.mlp = nn.Conv2d(c_in_total, c_out, kernel_size=(1, 1))
...
class ProbabilisticGWN(nn.Module):
    def __init__(
        self,
        num_nodes,
        input_features,
        ...
        predefined_adj = torch.tensor(predefined_adj, dtype=torch.float32)
        station_weights = torch.tensor(station_weights, dtype=torch.float32)

        self.register_buffer("predefined_adj", predefined_adj)
        self.register_buffer("predefined_adj_t", predefined_adj.T)
        self.register_buffer("station_weights", station_weights)

        support_len = 2

        if adaptive_adj:
            self.nodevec1 = nn.Parameter(torch.randn(num_nodes, node_emb_dim), requires_grad=True)
            self.nodevec2 = nn.Parameter(torch.randn(node_emb_dim, num_nodes), requires_grad=True)
            support_len += 1
        ...
        for _ in range(blocks):
            ...
            for i in range(layers):
                ...
                self.bn_layers.append(nn.BatchNorm2d(residual_channels))
        ...
        self.mean_head = nn.Conv1d(end_channels, 1, kernel_size=1)
        self.raw_var_head = nn.Conv1d(end_channels, 1, kernel_size=1)

    def build_supports(self):
        supports = [self.predefined_adj, self.predefined_adj_t]
    ...
    def forward(self, x_grid, x_global):
        ...
        x_global_expand = x_global.unsqueeze(2).repeat(1, 1, n, 1)
        x = torch.cat([x_grid, x_global_expand], dim=-1)
        ...
        for _ in range(self.blocks):
            for _ in range(self.layers):
                ...
                dilation = self.filter_convs[layer_idx].dilation[1]
                pad_len = dilation * (self.kernel_size - 1)

                x_padded = F.pad(x, (pad_len, 0, 0, 0))

                filter_out = torch.tanh(self.filter_convs[layer_idx](x_padded))
                gate_out = torch.sigmoid(self.gate_convs[layer_idx](x_padded))
                ...
                x = self.gconv_layers[layer_idx](x, supports)
        ...
        # Spatial readout from 9 ECMWF grid nodes to the bridge-site anemometer.
        # h: [B, C, N, 1]
        # station_weights: [N]
        h = torch.einsum("bcnt,n->bct", h, self.station_weights)

        h = F.relu(self.end_conv_1(h))
        h = F.dropout(h, self.dropout, training=self.training)
        h = F.relu(self.end_conv_2(h))

        mu = self.mean_head(h).squeeze(-1)
        raw_var = self.raw_var_head(h).squeeze(-1)
...
def train_one_horizon(cfg, transformed_data, scaler_y, horizon_id, run_seed, run_output_dir):
    lead_steps = horizon_id * cfg.samples_per_hour
    lead_hours = horizon_id

    horizon_dir = os.path.join(run_output_dir, f"Horizon_{horizon_id:02d}_step_{lead_hours:02d}h_ahead")
    ...
    best_model_path = os.path.join(horizon_dir, f"best_probabilistic_gwn_seed_{run_seed}_horizon_{horizon_id:02d}.pt")

    print("\n" + "=" * 80)
    print(f"Seed {run_seed} | Training Horizon {horizon_id:02d}: {lead_hours} h ahead, lead_steps = {lead_steps} samples")
    ...
    history_df.to_excel(
        os.path.join(horizon_dir, f"training_history_seed_{run_seed}_horizon_{horizon_id:02d}.xlsx"),
        index=False
    )
    ...
    # Evaluation and result export must use deterministic sample order.
    # The training loader may use shuffle=True, but prediction results must be
    # generated with shuffle=False so that Target_time, Observed, and predictions
    # are aligned with sample_meta for Train, Validation, and Test.
    eval_loaders = {}
    for split_name in ["Train", "Validation", "Test"]:
    ...
    for split_name in ["Train", "Validation", "Test"]:
        y_norm, mu_norm, var_norm = predict(model, eval_loaders[split_name], cfg.device)
        ...
        pred_path = os.path.join(
            horizon_dir,
            f"{split_name}_prediction_results_seed_{run_seed}_horizon_{horizon_id:02d}.xlsx"
        )
        pred_df.to_excel(pred_path, index=False)

        prediction_records[split_name] = pred_df

        print(f"\nSeed {run_seed}, Horizon {horizon_id:02d}, {split_name} metrics")
        for k, v in metrics.items():
            if isinstance(v, float):
                print(f"{k}: {v:.6f}")
            elif k in ["Seed", "Split", "Horizon_step", "Lead_hours", "Effective_sample_size", "Best_epoch"]:
                print(f"{k}: {v}")
    ...
    summary_df.to_excel(
        os.path.join(horizon_dir, f"metrics_summary_seed_{run_seed}_horizon_{horizon_id:02d}.xlsx"),
        index=False
    )
...
def save_static_information(cfg, raw_data, scaler_grid, scaler_global, scaler_y):
    ...
    for split_name, data in raw_data.items():
        ...
        split_info.append({
            ...
            "Random_seeds": str(cfg.random_seeds),
            "Number_of_random_seeds": len(cfg.random_seeds),
...
def save_combined_outputs(cfg, all_metrics, all_predictions, output_dir=None):
    ...
    metrics_all_df.to_excel(
        os.path.join(output_dir, "metrics_summary_all_horizons_all_splits.xlsx"),
        index=False
    )
    ...
    for split_name in ["Train", "Validation", "Test"]:
        ...
        split_long_df.to_excel(
            os.path.join(output_dir, f"{split_name}_prediction_results_all_horizons_long.xlsx"),
            index=False
        )
        ...
        wide_df.to_excel(
            os.path.join(output_dir, f"{split_name}_prediction_results_all_horizons_wide.xlsx"),
            index=False
        )

    test_metrics = metrics_all_df[metrics_all_df["Split"] == "Test"].copy()
    test_metrics.to_excel(
        os.path.join(output_dir, "Test_metrics_summary_all_horizons.xlsx"),
        index=False
    )


def train_one_seed_all_horizons(cfg, transformed_data, scaler_y, run_seed):
    run_output_dir = os.path.join(cfg.output_dir, f"seed_{run_seed}")
    ...
    save_combined_outputs(cfg, all_metrics, all_predictions, output_dir=run_output_dir)

    history_all_df = pd.concat(all_histories, axis=0, ignore_index=True)
    history_all_df.to_excel(
        os.path.join(run_output_dir, "training_history_all_horizons.xlsx"),
        index=False
    )

    metrics_all_df = pd.concat(all_metrics, axis=0, ignore_index=True)
    metrics_all_df.to_excel(
        os.path.join(run_output_dir, f"seed_{run_seed}_metrics_summary_all_horizons_all_splits.xlsx"),
        index=False
    )

    print("\nSeed-level results saved to:")
    print(run_output_dir)

    print(f"\nFinal Test summary for seed {run_seed}:")
    cols = [
        "Seed",
        "Horizon_step",
        "Lead_hours",
    ...
    return metrics_all_df, history_all_df


def summarize_robustness(seed_metrics_df, cfg):
    metric_cols = ["RMSE", "MAE", "MAPE_percent", "R2", "PICP_percent", "PINAW", "CRPS", "NLL"]
    group_cols = ["Horizon_step", "Lead_hours", "Lead_samples", "Split"]

    rows = []

    for group_values, group_df in seed_metrics_df.groupby(group_cols):
        ...
        for metric in metric_cols:
            ...
            if len(values) > 1:
                ...
                std_value = 0.0
            cv_percent = float(std_value / (abs(mean_value) + 1e-12) * 100.0)
    ...
    robustness_long_df = pd.DataFrame(rows)

    mean_std_wide = robustness_long_df.pivot_table(
        index=group_cols,
        columns="Metric",
        values="Mean±Std",
        aggfunc="first"
    ...
    cv_wide = robustness_long_df.pivot_table(
        index=group_cols,
        columns="Metric",
        values="CV_percent",
        aggfunc="first"
    ...
    full_wide_rows = []
    for group_values, group_df in robustness_long_df.groupby(group_cols):
        row = dict(zip(group_cols, group_values))
        for _, metric_row in group_df.iterrows():
            metric = metric_row["Metric"]
            row[f"{metric}_Mean"] = metric_row["Mean"]
            row[f"{metric}_Std"] = metric_row["Std"]
            row[f"{metric}_CV_percent"] = metric_row["CV_percent"]
            row[f"{metric}_Mean±Std"] = metric_row["Mean±Std"]
        full_wide_rows.append(row)

    full_wide_df = pd.DataFrame(full_wide_rows)

    robustness_long_df.to_excel(
        os.path.join(cfg.output_dir, "robustness_summary_mean_std_cv_long.xlsx"),
        index=False
    )

    mean_std_wide.to_excel(
        os.path.join(cfg.output_dir, "robustness_summary_mean_std_wide.xlsx"),
        index=False
    )

    cv_wide.to_excel(
        os.path.join(cfg.output_dir, "robustness_summary_cv_percent_wide.xlsx"),
        index=False
    )

    full_wide_df.to_excel(
        os.path.join(cfg.output_dir, "robustness_summary_full_wide.xlsx"),
        index=False
    )

    test_long_df = robustness_long_df[robustness_long_df["Split"] == "Test"].copy()
    test_long_df.to_excel(
        os.path.join(cfg.output_dir, "Test_robustness_summary_mean_std_cv_long.xlsx"),
        index=False
    )

    test_full_wide_df = full_wide_df[full_wide_df["Split"] == "Test"].copy()
    test_full_wide_df.to_excel(
        os.path.join(cfg.output_dir, "Test_robustness_summary_full_wide.xlsx"),
        index=False
    )

    return robustness_long_df, mean_std_wide, cv_wide, full_wide_df
...
def train_all_horizons(cfg):
    ...
    print(f"Random seeds for robustness analysis: {cfg.random_seeds}")
    ...
    for run_seed in cfg.random_seeds:
        print("\n" + "=" * 80)
        print(f"Running GWN robustness experiment with random seed = {run_seed}")
        print("=" * 80)

        seed_metrics_df, seed_history_df = train_one_seed_all_horizons(
            cfg=cfg,
            transformed_data=transformed_data,
            scaler_y=scaler_y,
            run_seed=run_seed,
        ...
        all_seed_metrics.append(seed_metrics_df)
        all_seed_histories.append(seed_history_df)

    seed_metrics_all_df = pd.concat(all_seed_metrics, axis=0, ignore_index=True)
    seed_metrics_all_df.to_excel(
        os.path.join(cfg.output_dir, "robustness_seed_level_metrics_all_horizons_all_splits.xlsx"),
        index=False
    )

    seed_histories_all_df = pd.concat(all_seed_histories, axis=0, ignore_index=True)
    seed_histories_all_df.to_excel(
        os.path.join(cfg.output_dir, "robustness_training_history_all_seeds_all_horizons.xlsx"),
        index=False
    )

    robustness_long_df, mean_std_wide, cv_wide, full_wide_df = summarize_robustness(
        seed_metrics_all_df,
        cfg
    )

    print("\nAll robustness results saved to:")
    ...
    print(seed_metrics_all_df[seed_metrics_all_df["Split"] == "Test"][cols])

    print("\nRobustness summary, mean ± std:")
    print(mean_std_wide[mean_std_wide["Split"] == "Test"])

    print("\nRobustness summary, CV percent:")
    print(cv_wide[cv_wide["Split"] == "Test"])

    return seed_metrics_all_df, robustness_long_df, mean_std_wide, cv_wide, full_wide_df


if __name__ == "__main__":
    set_seed(cfg.seed)
    seed_metrics_df, robustness_long_df, mean_std_wide, cv_wide, full_wide_df = train_all_horizons(cfg)
