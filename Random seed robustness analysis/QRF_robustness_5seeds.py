...
class Config:
    ...
    output_dir = os.path.join(root_dir, "QRF_Dataset1_robustness_5seeds_results")
...
def train_one_horizon(cfg, transformed_data, scaler_y, horizon_id, run_seed, run_output_dir):
    ...
    model = QuantileRandomForest(
        ...
        n_jobs=cfg.n_jobs,
        random_state=run_seed + horizon_id,
    )
    ...
    training_info_df.to_excel(
        os.path.join(horizon_dir, f"qrf_training_information_seed_{run_seed}_horizon_{horizon_id:02d}.xlsx"),
        index=False
    )
    ...
    feature_importance_df.to_excel(
        os.path.join(horizon_dir, f"feature_importance_seed_{run_seed}_horizon_{horizon_id:02d}.xlsx"),
        index=False
    )
...
def train_one_seed_all_horizons(cfg, transformed_data, scaler_y, run_seed):
    ...
    for horizon_id in cfg.horizon_steps:
        ...
        all_training_info.append(training_info_df.assign(Seed=run_seed, Horizon_step=horizon_id))
    ...
    training_info_all_df.to_excel(
        os.path.join(run_output_dir, "qrf_training_information_all_horizons.xlsx"),
        index=False
    )
    ...
    return metrics_all_df, training_info_all_df
...
def train_all_horizons(cfg):
    ...
    all_seed_metrics = []
    all_seed_training_info = []

    for run_seed in cfg.random_seeds:
        print("\n" + "=" * 80)
        print(f"Running QRF robustness experiment with random seed = {run_seed}")
        print("=" * 80)

        seed_metrics_df, seed_training_info_df = train_one_seed_all_horizons(
            cfg=cfg,
            transformed_data=transformed_data,
            scaler_y=scaler_y,
            run_seed=run_seed,
        ...
        all_seed_training_info.append(seed_training_info_df)
    ...
    seed_training_info_all_df = pd.concat(all_seed_training_info, axis=0, ignore_index=True)
    seed_training_info_all_df.to_excel(
        os.path.join(cfg.output_dir, "robustness_qrf_training_information_all_seeds_all_horizons.xlsx"),
        index=False
    )
