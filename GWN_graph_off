...
class Config:
    ...
    output_dir = os.path.join(root_dir, "GWN_Dataset1_graph_off_results")
    ...
    gcn_order = 2
    dropout = 0.20
    # Graph-off ablation: temporal convolution is retained, while graph convolution
    # based on both predefined and adaptive adjacency matrices is disabled.
...
# =========================================================
# 5. Probabilistic Graph WaveNet graph-off ablation
# =========================================================
...
class ProbabilisticGWN(nn.Module):
    def __init__(
        ...
        self.adaptive_adj = adaptive_adj if use_graph_conv else False
        ...
        if self.use_graph_conv and self.adaptive_adj:
        ...
        for _ in range(blocks):
            ...
            for i in range(layers):
                ...
                residual_input_channels = gcn_channels if self.use_graph_conv else dilation_channels
    ...
    def forward(self, x_grid, x_global):
        ...
        skip = 0
        supports = self.build_supports() if self.use_graph_conv else None
...
def build_model(cfg):
    model = ProbabilisticGWN(
        ...
        dropout=cfg.dropout,
        use_graph_conv=cfg.use_graph_conv,
...
def train_one_horizon(cfg, transformed_data, scaler_y, horizon_id):
    ...
    print(f"Graph convolution used = {cfg.use_graph_conv}, adaptive adjacency used = {cfg.adaptive_adj}")
...
def save_static_information(cfg, raw_data, scaler_grid, scaler_global, scaler_y):
    ...
    for split_name, data in raw_data.items():
        ...
        split_info.append({
            ...
            "Graph_convolution_used": cfg.use_graph_conv,
            "Adaptive_adjacency_used": cfg.adaptive_adj,
